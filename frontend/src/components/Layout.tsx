...
export default function Layout(props: { children?: JSX.Element }) {
  ...
  return (
    <div class={`min-h-screen ${theme() === 'dark' ? 'dark bg-gray-900 text-white' : 'bg-gray-50 text-gray-900'}`}>
      <nav class="bg-white dark:bg-gray-800 shadow-md">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div class="flex justify-between h-16">
            <div class="flex">
              <div class="flex-shrink-0 flex items-center">
                <span class="text-xl font-bold text-blue-600 dark:text-blue-400">CRUD App</span>
              </div>
            ...
            <div class="flex items-center">
               ...
               >
                 {theme() === 'dark' ? <SunIcon class="w-5 h-5" /> : <MoonIcon class="w-5 h-5" />}
               </button>
            </div>
          </div>
        </div>
