...
import { SortIcon, SortUpIcon, SortDownIcon, EditIcon, DeleteIcon, CloseIcon, BuildingIcon } from '../components/Icons';
...
export default function ObrasPage() {
  ...
  const handleCreate = () => {
    setEditingId(null);
    setFormData({ nombre: '', valor_contrato: 0, estado: 'activa' });
    setError(null);
    setShowModal(true);
  };
    ...
    <div class="p-6">
      <div class="flex justify-between items-center mb-6">
         <div class="flex items-center space-x-3">
           ...
           <h1 class="text-3xl font-bold text-gray-900 dark:text-white">Obras</h1>
         </div>
        <button
       ...
       {obras.error && (
         <div class="p-4 bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 rounded-lg mb-4">
           <p class="text-red-800 dark:text-red-200">Error al cargar obras: {obras.error.message}</p>
         </div>
       )}
          ...
          <div class="overflow-x-auto">
            {/* Header */}
             <div class={`grid grid-cols-4 gap-3 px-6 py-3 text-center text-xs font-medium uppercase tracking-wider text-gray-500 dark:text-gray-300 ${theme() === 'dark' ? 'bg-gray-800' : 'bg-gray-100'} border-b border-gray-200 dark:border-gray-700`}>
               ...
               <div class="cursor-pointer flex items-center justify-center" onClick={() => handleSort('valor_contrato')}>
                 <span>Valor Contrato</span>
                 {SortIconComponent('valor_contrato')}
               </div>
               <div class="cursor-pointer flex items-center justify-center" onClick={() => handleSort('estado')}>
                 <span>Estado</span>
            ...
            <div>
              <For each={filteredAndSorted()}>
                 {(obra) => (
                   <div class="grid grid-cols-4 text-center gap-3 px-6 py-4 hover:bg-gray-50 dark:hover:bg-gray-800 group relative items-center">
            ...
            <form onSubmit={handleSubmit} class="p-6 space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  onInput={(e) => handleInputChange('valor_contrato', parseFloat(e.currentTarget.value))}
                />
              </div>
              <div>
                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
